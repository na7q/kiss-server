...
def listen_and_forward(destination_socket, client_sockets):
    while True:
        try:
            ...
            if not data:
                raise ConnectionError("Connection to the destination server is lost")
            ...
            # Reconnect to the destination server
            while True:
                try:
                    print("Reconnecting to the destination server...")
                    destination_socket.close()
                    destination_socket = connect_to_destination()
                    print("Destination connected")
                    break  # Break the reconnection loop if successful
                except Exception as e:
                    print(f"Reconnection failed. Retrying in 5 seconds. Error: {e}")
